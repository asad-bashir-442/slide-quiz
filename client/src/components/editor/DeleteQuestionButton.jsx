    ...
    return (
        <>
            <button
                ...
                onClick={() => document.getElementById(`delete_question_modal/${question?.id}`).showModal()}
            >
                Delete Question
            </button>
            <dialog id={`delete_question_modal/${question?.id}`} className="modal">
                <div className="modal-box">
                    <h3 className="font-bold text-lg" title={question?.description}>
                        Are you sure you want to delete "{truncateText(question?.description, 15)}"
                    </h3>
                    ...
                    <div className="modal-action">
                        <form onSubmit={handleSubmit} className="max-[900px]:w-full">
                            <button
                                ...
                                onClick={() => document.getElementById(`delete_question_modal/${question?.id}`).close()}
                            >
                                ✕
                            </button>

                            <div className="modal-action max-[900px]:block max-[900px]:w-full">
                                <button onClick={() => document.getElementById(`delete_question_modal/${question?.id}`).close()} type="reset" className="btn max-[900px]:mb-4 max-[900px]:w-full">
                                    Cancel
                                </button>
