...
export function EditQuestonButton({
  quizId,
  questionId,
  description,
  points,
  ...
  return (
    <>
      <Pencil
        onClick={handleClick}
        className="h-[0.8em] hover:opacity-40 cursor-pointer"
      />
      <dialog
        onClick={(e) => e.stopPropagation()}
        id={`edit_question_modal/${questionId}`}
        className="modal"
      >
          ...
          <form onSubmit={handleSubmit} className="modal-action flex-col">
            ...
            <div className="modal-action max-[900px]:block max-[900px]:w-full">
              <button
                onClick={() =>
                  document
                    .getElementById(`edit_question_modal/${questionId}`)
                    .close()
                }
                type="reset"
