        ...
        try {
            ...
            if (data?.statusCode == 200) {
                if (data.data.length === 0) {
                    ...
                    setQuizzes((prev) => {
                        const newQuizzes = data.data.filter((newQuiz) => !prev.some((existingQuiz) => existingQuiz.id === newQuiz.id));
        ...
        <>
            ...
            {quizzes.length > 0 && (
                <div className="flex justify-center mt-4 pb-8">
                    <button onClick={loadMore} disabled={!hasMore || loadingMore} className={`btn btn-outline max-[900px]:w-[90%] ${!hasMore ? "btn-disabled" : "btn-primary"}`}>
                        {loadingMore ? (
                            <>
