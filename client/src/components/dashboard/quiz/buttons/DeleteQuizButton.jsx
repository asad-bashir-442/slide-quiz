...
export function DeleteQuizButton({ id, quizName, setQuizzes }) {
    ...
    return (
        <>
            ...
            <dialog id={`delete_quiz_modal/${id}`} className="modal" onClick={(e) => e.stopPropagation()}>
                <div className="modal-box">
                    ...
                    <div className="modal-action">
                        <form onSubmit={handleSubmit} className="max-[900px]:w-full">
                            <button type="button" className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2" onClick={() => document.getElementById(`delete_quiz_modal/${id}`).close()}>
                                ✕
                            </button>
